...
@WebSocketGateway({
  cors: { origin: '*' },
  path: '/socket.io', // 여기서 경로를 '/ws'로 설정
})
@Injectable()
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleGetMessages(
    ...
    console.log(
      `Sent previous messages for room ${roomId} to socket ${client.id}`,
    );
  }
  ...
  async handleSendMessage(
    @MessageBody()
    data: { chatId: string; content: string; userId: number; username: string },
    @ConnectedSocket() client: Socket,
  ) {
